...
char login(struct LoginToken login_token) {
	...
	if (login_token.choice == 3) {
		...
		while(1) {
			...
			if (temp.stud_id == login_token.userid && strcmp(temp.password, login_token.password) == 0 && temp.activity_stat == 1) {
				setlock(lock, fd, F_UNLCK, SEEK_CUR, 0, sizeof(temp));
				return 's';
			}
			setlock(lock, fd, F_UNLCK, SEEK_CUR, 0, sizeof(temp));
...
int unenroll_from_course(int student_id, int course_id) {
	...
	while (1) {
		...
		if (read_stat == 0) {
			setlock(lock, fd, F_UNLCK, SEEK_CUR, 0, sizeof(temp));
			printf("Course not found in student\n");
			return -1;
		} else if (temp.stud_id == student_id) {
			...
			lseek(fd, -1 * (sizeof(temp)), SEEK_CUR);
			printf("New student course count: %d\n", temp.num_courses);
			int write_stat = write(fd, &temp, sizeof(temp));
			setlock(lock, fd, F_UNLCK, SEEK_CUR, 0, sizeof(temp));
			if (write_stat < 0) {
				perror("Writing Failed");
	...
	printf("Everything done in unenroll_from_course\n");
	return 1;
}
...
int main() {
    ...
    while (1) {
        ...
        if (pid == -1) {
            ...
            close(server);
            int client_stat = handle_client(client);
            if (client_stat < 0) {
                perror("Server Terminated Unexpectedly");
                exit(-1);
            }
            exit(0);
        } else {
